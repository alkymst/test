<?php
/*
Plugin Name: GForms Highrise CRM
Plugin URI:
Description: Integrates Gravity Forms with Highrise CRM
Version: 1.0
Author: RiotLabs
Author URI: http://riotlabs.com

------------------------------------------------------------------------
Copyright 2015 RiotLabs
	...
	public static function export_feed($entry, $form, $feed, $api){
		...
		if( !empty($params) )
		{
			...
			try
			{
				...
				// HACK: Hardcoded to Steve's user
				if (1==1) {
					$task = new HighriseTask($api);
					$task->setOwnerId(430726); // steve
					$task->setSubjectType("Party");
					$task->setSubjectId($id);
					$task->setCategoryId(2634797); // Lead
					$task->setNotify(true);
					$task->setFrame('today');
					$task->save();
				}
