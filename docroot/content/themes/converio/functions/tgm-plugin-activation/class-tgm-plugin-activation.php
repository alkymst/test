    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            $this->strings = array(
                ...
                'oops'                           => esc_attr__( 'Something went wrong.', 'tgmpa' ),
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '">' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = esc_attr__( 'Private Repository', 'tgmpa' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = esc_attr__( 'Pre-Packaged', 'tgmpa' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = esc_attr__( 'WordPress Repository', 'tgmpa' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? esc_attr__( 'Required', 'tgmpa' ) : esc_attr__( 'Recommended', 'tgmpa' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', esc_attr__( 'Not Installed', 'tgmpa' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', esc_attr__( 'Installed But Not Activated', 'tgmpa' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'tgmpa' ) . ' %2$s">' . esc_attr__( 'Install', 'tgmpa' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'tgmpa' ) . ' %2$s">' . esc_attr__( 'Activate', 'tgmpa' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( esc_attr__( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'tgmpa' ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'status' => esc_attr__( 'Status', 'tgmpa' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => esc_attr__( 'Install', 'tgmpa' ),
                'tgmpa-bulk-activate' => esc_attr__( 'Activate', 'tgmpa' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . esc_attr__( 'No plugins are available to be installed at this time.', 'tgmpa' ) . '</p></div>';
                    return false;
                }
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . esc_attr__( 'No plugins are available to be activated at this time.', 'tgmpa' ) . '</p></div>';
                    return false;
                }
        ...
        if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                ...
                public function install_strings() {
                    ...
                    $this->strings['no_package']          = esc_attr__( 'Install package not available.', 'tgmpa' );
                    $this->strings['downloading_package'] = esc_attr__( 'Downloading install package from <span class="code">%s</span>&#8230;', 'tgmpa' );
                    $this->strings['unpack_package']      = esc_attr__( 'Unpacking the package&#8230;', 'tgmpa' );
                    $this->strings['installing_package']  = esc_attr__( 'Installing the plugin&#8230;', 'tgmpa' );
                    $this->strings['process_failed']      = esc_attr__( 'Plugin install failed.', 'tgmpa' );
                    $this->strings['process_success']     = esc_attr__( 'Plugin installed successfully.', 'tgmpa' );
            ...
            class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function add_strings() {
                    ...
                    if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                        ...
                        $this->upgrader->strings['skin_update_successful']    = esc_attr__( '%1$s installed and activated successfully.', 'tgmpa' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_attr__( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_attr__( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
                    ...
                    // Default installation strings.
                        ...
                        $this->upgrader->strings['skin_update_successful']    = esc_attr__( '%1$s installed successfully.', 'tgmpa' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_attr__( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_attr__( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
                ...
                public function bulk_footer() {
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '">' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    }
